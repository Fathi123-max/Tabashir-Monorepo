...
class ProfileRepositoryImpl implements ProfileRepository {
  ...
  Future<UserProfileResponse?> getCachedProfile() async {
    try {
      ...
      AppLogger.error(
        '[PROFILE_REPO] Error getting cached profile: $e',
        tag: 'Profile',
        error: e,
      );
    }
  ...
  Future<UserProfileResponse> getUserProfile() async {
    ...
    try {
      ...
      if (cachedProfile != null) {
        AppLogger.debug(
          '[PROFILE_REPO] ✅ Found cached profile',
          tag: 'Profile',
        );
      } else {
        AppLogger.debug(
          '[PROFILE_REPO] No cached profile found',
          tag: 'Profile',
        );
      }
      ...
      // Fetch from API
      AppLogger.debug(
        '[PROFILE_REPO] Calling UserApiService.getUserProfile()...',
        tag: 'Profile',
      );
      AppLogger.debug(
        '[PROFILE_REPO] Using comprehensive unified API: /api/v1/users/me',
        tag: 'Profile',
      );
      ...
      AppLogger.debug(
        '[PROFILE_REPO] Response status: ${response.response.statusCode}',
        tag: 'Profile',
      );
      ...
      if (response.response.statusCode == 200 ||
        ...
        AppLogger.debug(
          '[PROFILE_REPO] ✅ Comprehensive profile loaded successfully',
          tag: 'Profile',
        );
        AppLogger.debug(
          '[PROFILE_REPO] User data: ${response.data.user.name}',
          tag: 'Profile',
        );
        AppLogger.debug(
          '[PROFILE_REPO] User type: ${response.data.user.userType}',
          tag: 'Profile',
        );
        AppLogger.debug(
        ...
        // Save to local cache
        AppLogger.debug(
          '[PROFILE_REPO] Saving profile to local cache...',
          tag: 'Profile',
        );
        await _saveToLocal(response.data);
        AppLogger.debug(
          '[PROFILE_REPO] ✅ Profile saved to cache',
          tag: 'Profile',
        );

        AppLogger.debug(
          '########## [PROFILE_REPO] SUCCESS ##########\n\n',
          tag: 'Profile',
        );
        ...
        if (cachedProfile != null) {
          AppLogger.error(
            '[PROFILE_REPO] Using cached profile due to API error',
            tag: 'Profile',
          );
          AppLogger.debug(
            '########## [PROFILE_REPO] SUCCESS (CACHE) ##########\n\n',
            tag: 'Profile',
          );
          return _convertFromLocal(cachedProfile);
        }
      ...
      if (e.response != null) {
        AppLogger.debug(
          '[PROFILE_REPO] Response status: ${e.response?.statusCode}',
          tag: 'Profile',
        );
        AppLogger.debug(
          '[PROFILE_REPO] Response data: ${e.response?.data}',
          tag: 'Profile',
        );
      }
      ...
      if (cachedProfile != null) {
        AppLogger.error(
          '[PROFILE_REPO] Returning cached profile due to error',
          tag: 'Profile',
        );
        AppLogger.debug(
          '########## [PROFILE_REPO] SUCCESS (CACHE) ##########\n\n',
  ...
  Future<void> updateProfile({
    ...
    try {
      AppLogger.debug(
        '[PROFILE_REPO] Calling UserApiService.updateProfile()...',
        tag: 'Profile',
      );
      AppLogger.debug(
        '[PROFILE_REPO] Data: ${profileUpdate.toJson()}',
        tag: 'Profile',
      );
      ...
      if (response.response.statusCode == 200 ||
        ...
        AppLogger.debug(
          '[PROFILE_REPO] Response data: ${response.data}',
          tag: 'Profile',
        );

        // Update local cache with new data
        AppLogger.debug(
          '[PROFILE_REPO] Updating local cache...',
          tag: 'Profile',
        );
  ...
  Future<void> updateClient({
    ...
    try {
      AppLogger.debug(
        '[PROFILE_REPO] Calling TabashirApiService.updateClient()...',
        tag: 'Profile',
      );
      ...
      if (response.response.statusCode == 200 ||
          response.response.statusCode == 201) {
        AppLogger.debug(
          '[PROFILE_REPO] ✅ Client updated successfully',
          tag: 'Profile',
        );
        AppLogger.debug(
          '########## [PROFILE_REPO] SUCCESS ##########\n\n',
  ...
  Future<AiClientData?> getClient() async {
    ...
    try {
      ...
      if (response.response.statusCode == 200) {
        ...
        AppLogger.debug(
          '[PROFILE_REPO] Client profile not found in AI DB',
          tag: 'Profile',
        );
        return null;
      } else {
      ...
      AppLogger.error(
        '[PROFILE_REPO] ❌ DioException: ${e.message}',
        tag: 'Profile',
      );
      return null; // Non-fatal - AI profile may not exist yet
    } catch (e) {
      AppLogger.error(
        '[PROFILE_REPO] ❌ Error fetching client: $e',
        tag: 'Profile',
        error: e,
      );
      return null;
  ...
  Future<void> deleteAccount() async {
    ...
    try {
      AppLogger.debug(
        '[PROFILE_REPO] Calling AuthApiService.deleteAccount()...',
        tag: 'Profile',
      );
      ...
      // Clear local cache
      AppLogger.debug(
        '[PROFILE_REPO] Clearing local profile cache...',
        tag: 'Profile',
      );
