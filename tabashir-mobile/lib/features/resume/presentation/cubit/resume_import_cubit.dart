...
class ResumeImportCubit extends Cubit<ResumeImportState> {
  ...
  Future<void> pickResumeFile() async {
    ...
    if (!isAuthenticated) {
      AppLogger.debug(
        '🔴 [RESUME_IMPORT] User not authenticated - cannot upload resume',
        tag: 'Resume',
      );
      emit(
    ...
    try {
      AppLogger.debug(
        '🔵 [RESUME_IMPORT] Opening file picker...',
        tag: 'Resume',
      );
      ...
      if (result != null) {
        AppLogger.debug(
          '🔵 [RESUME_IMPORT] Files selected: ${result.files.length}',
          tag: 'Resume',
        );
        final file = result.files.single;
        AppLogger.debug(
          '🔵 [RESUME_IMPORT] Selected file details:',
          tag: 'Resume',
        );
        ...
        if (file.path == null) {
          AppLogger.debug(
            '🔴 [RESUME_IMPORT] File path is null',
            tag: 'Resume',
          );
        ...
        AppLogger.debug(
          '🔵 [RESUME_IMPORT] Checking if file exists at path...',
          tag: 'Resume',
        );
        final resumeFile = File(file.path!);
        final fileExists = await resumeFile.exists();
        AppLogger.debug(
          '🔵 [RESUME_IMPORT] File exists: $fileExists',
          tag: 'Resume',
        );
        ...
        AppLogger.debug(
          '🔵 [RESUME_IMPORT] User cancelled file selection',
          tag: 'Resume',
        );
        AppLogger.debug(
          '🔵 [RESUME_IMPORT] Emitting initial state',
          tag: 'Resume',
        );
      ...
      AppLogger.debug(
        '🔴 [RESUME_IMPORT] EXCEPTION occurred in pickResumeFile():',
        tag: 'Resume',
      );
      AppLogger.error(
        '🔴 [RESUME_IMPORT]',
    ...
    AppLogger.debug(
      '🔵 [RESUME_IMPORT] === pickResumeFile() COMPLETED ===\n',
      tag: 'Resume',
    );
  }
  ...
  Future<void> pasteResumeText(String resumeText) async {
    ...
    AppLogger.debug(
      '🔵 [RESUME_IMPORT] Text length: ${resumeText.length} characters',
      tag: 'Resume',
    );

    if (resumeText.trim().isEmpty) {
      AppLogger.debug(
        '🔴 [RESUME_IMPORT] ERROR: Resume text is empty',
        tag: 'Resume',
      );
    ...
    try {
      ...
      AppLogger.debug(
        '🟢 [RESUME_IMPORT] ✅ Resume text parsed successfully',
        tag: 'Resume',
      );
      AppLogger.debug(
        '🟢 [RESUME_IMPORT] Parsed data: $parsedData',
        tag: 'Resume',
      );

      AppLogger.debug(
        '🔵 [RESUME_IMPORT] Emitting success state',
        tag: 'Resume',
      );
      emit(const ResumeImportState.success());
      AppLogger.debug(
        '🟢 [RESUME_IMPORT] ✅ Successfully emitted success state',
        tag: 'Resume',
      );
    } catch (e, stackTrace) {
      AppLogger.error(
  ...
  Future<void> _uploadResume({
    ...
    try {
      AppLogger.debug(
        '🟡 [RESUME_IMPORT] Extracting structured data before upload...',
        tag: 'Resume',
      );
      ...
      try {
        ...
        AppLogger.debug(
          '🟢 [RESUME_IMPORT] Local parsing completed.',
          tag: 'Resume',
        );
      } catch (e) {
        AppLogger.error(
      ...
      AppLogger.debug(
        '🟢 [RESUME_IMPORT] ✅ uploadResume() completed',
        tag: 'Resume',
      );
      AppLogger.debug(
        '🟢 [RESUME_IMPORT] Upload result ID: ${result.id}',
        tag: 'Resume',
      );
      ...
      try {
        AppLogger.debug(
          '🟡 [RESUME_IMPORT] Triggering HomeCubit refresh...',
          tag: 'Resume',
        );
        await getIt<HomeCubit>().refreshHomeData();
      } catch (e) {
        AppLogger.error(
          '🔴 [RESUME_IMPORT] Failed to trigger HomeCubit refresh: $e',
          tag: 'Resume',
          error: e,
        );
      }
      ...
      AppLogger.debug(
        '🔴 [RESUME_IMPORT] StackTrace: $stackTrace',
        tag: 'Resume',
      );
