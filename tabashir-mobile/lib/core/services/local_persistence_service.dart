...
class LocalPersistenceService {
  ...
  static final LocalPersistenceService _instance =
      LocalPersistenceService._internal();
...
class SavedJobsRepository {
  ...
  Future<void> saveJob(String jobId) async {
    try {
      ...
      if (_apiService != null) {
        ...
        AppLogger.debug(
          '[SAVED_JOBS_REPO] Successfully saved job $jobId to API',
          tag: 'Service',
        );
      }
      ...
      AppLogger.error(
        '[SAVED_JOBS_REPO] Error saving job: $e',
        tag: 'Service',
        error: e,
      );
  ...
  Future<void> removeSavedJob(String jobId) async {
    try {
      AppLogger.debug(
        '[SAVED_JOBS_REPO] Removing saved job $jobId...',
        tag: 'Service',
      );
      ...
      if (_apiService != null) {
        ...
        AppLogger.debug(
          '[SAVED_JOBS_REPO] Successfully removed job $jobId from API',
          tag: 'Service',
        );
      }
      ...
      AppLogger.error(
        '[SAVED_JOBS_REPO] Error removing saved job: $e',
        tag: 'Service',
        error: e,
      );
      // Revert on failure
  ...
  Future<List<String>> getAllSavedJobIds() async {
    try {
      AppLogger.debug(
        '[SAVED_JOBS_REPO] getAllSavedJobIds() called',
        tag: 'Service',
      );
      ...
      if (_apiService != null) {
        ...
        if (response.success) {
          ...
          AppLogger.debug(
            '[SAVED_JOBS_REPO] API returned ${rawSavedJobs.length} items',
            tag: 'Service',
          );
      ...
      AppLogger.debug(
        '[SAVED_JOBS_REPO] Falling back to local storage...',
        tag: 'Service',
      );
      ...
      AppLogger.error(
        '[SAVED_JOBS_REPO] ❌ Error fetching saved jobs: $e',
        tag: 'Service',
        error: e,
      );
  ...
  Future<void> syncJobs(List<Map<String, dynamic>> jobs) async {
    ...
    AppLogger.debug(
      '[SAVED_JOBS_REPO] Syncing ${jobs.length} jobs to local cache',
      tag: 'Service',
    );
